...
class CurriculumManager_OptionsManager {
    ...
    public function settingsPage() {
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'curriculum-manager'));
        }
        ...
        ?>
        <div class="wrap">
            <h2><?php _e('System Settings', 'curriculum-manager'); ?></h2>
            <table class="form-table"><tbody>
            <tr><td><?php _e('System', 'curriculum-manager'); ?></td><td><?php echo php_uname(); ?></td></tr>
            <tr><td><?php _e('PHP Version', 'curriculum-manager'); ?></td>
                <td><?php echo phpversion(); ?>
                <?php
                if (version_compare('5.2', phpversion()) > 0) {
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    _e('(WARNING: This plugin may not work properly with versions earlier than PHP 5.2)', 'curriculum-manager');
                    echo '</span>';
                }
                ?>
                </td>
            </tr>
            <tr><td><?php _e('MySQL Version', 'curriculum-manager'); ?></td>
                <td><?php echo $this->getMySqlVersion() ?>
                    <?php
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    if (version_compare('5.0', $this->getMySqlVersion()) > 0) {
                        _e('(WARNING: This plugin may not work properly with versions earlier than MySQL 5.0)', 'curriculum-manager');
                    }
                    echo '</span>';
                    ?>
                </td>
            ...
            <h2><?php echo $this->getPluginDisplayName(); echo ' '; _e('Settings', 'curriculum-manager'); ?></h2>
            ...
            <?php settings_fields($settingsGroup); ?>
                ...
                <p class="submit">
                    <input type="submit" class="button-primary"
                           value="<?php _e('Save Changes', 'curriculum-manager') ?>"/>
                </p>
            </form>
        </div>
        <?php
    ...
    protected function getOptionValueI18nString($optionValue) {
        switch ($optionValue) {
            case 'true':
                return __('true', 'curriculum-manager');
            case 'false':
                return __('false', 'curriculum-manager');

            case 'Administrator':
                return __('Administrator', 'curriculum-manager');
            case 'Editor':
                return __('Editor', 'curriculum-manager');
            case 'Author':
                return __('Author', 'curriculum-manager');
            case 'Contributor':
                return __('Contributor', 'curriculum-manager');
            case 'Subscriber':
                return __('Subscriber', 'curriculum-manager');
            case 'Anyone':
                return __('Anyone', 'curriculum-manager');
        }
        return $optionValue;
    }
